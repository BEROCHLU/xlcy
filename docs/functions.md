...
このファイルでは、サイドパネルの Grid.js テーブルを初期化し、表示・非表示の同期処理を定義しています。
...
### `beginDrawCytoscape(file, isRemote = false)`
...
8. `isRemote` が true の場合、同名 JSON があれば自動復元する
9. 最後に全コメントを表示する
...
`REMOTE FILE` の選択値から `./static/uploads/<filename>` を fetch し、成功したら `beginDrawCytoscape(file, true)` を呼びます。リモート読み込みなので、同名 JSON があれば描画後に自動復元されます。
...
`nav` の `open-menu` クラスを切り替え、ボタン文言を `メニュー` / `閉じる` に切り替えます。
