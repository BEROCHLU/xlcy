...
- 生成 AI レビュー用の `public/review.html` をインライン更新できます。
...
`npm install` が必要なのは、`npm run build:license` または `npm run build:inline` を実行する場合だけです。
...
`public/static/licenses/` 配下のライセンスファイルを結合し、`public/static/licenses/LICENSE.txt` を更新します。
...
`public/static/js/` の JavaScript を決められた順序で結合・圧縮し、`public/review.html` の `<script>` 内へ流し込みます。通常のアプリ実行には不要です。
...
サードパーティライブラリのライセンスは `public/static/licenses/LICENSE.txt` にまとめています。
