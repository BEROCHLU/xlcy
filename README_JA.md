...
エッジの探索コストは `edge.weight`、未指定なら `edge.value` です。経路が決まった後、経路上のノードの `node.weight` が結果に加算されます。`node.weight` が空欄（未指定）の場合は `0` として計算されます。独自のコストを設定したい場合は任意の半角数値を入力してください。
...
経路上のノードやエッジの `weight` または `value` に、数値以外の文字が入っている可能性があります。すべて半角数値であることを確認してください（なお、ノードの `weight` が空欄の場合は自動的に `0` として扱われます）。
