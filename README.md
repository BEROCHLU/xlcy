...
│   └── image2.png
├── docs/
│   ├── functions.md                      # JavaScript function overview
...
### `public/static/js/` Contents

JS source files for bundling. Concatenated in the order defined in `gulpfile.js`.

| File | Role |
| --- | --- |
| `server-fetch.js` | Async communication with PHP for Excel lists & layout saving |
| `workbook-utils.js` | Converts Excel sheet data to Cytoscape node/edge structures |
| `gridjs-updater.js` | Updates side-panel Grid.js tables & toggles element visibility |
| `tippy-attach.js` | Creates & displays Tippy.js tooltips from `memo` column |
| `cytoscape-gestures.js` | Click, tap & context menu gesture handlers for Cytoscape |
| `jqtab.js` | Tab switching UI |
| `entry.js` | Main flow: graph rendering, file loading, layout save/restore, path calculation |
